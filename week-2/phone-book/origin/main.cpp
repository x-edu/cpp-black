...
void TestSerialization() {
  ...
  AddressBookSerialize::ContactList list;
  ASSERT(list.ParseFromIstream(&input));
  ASSERT_EQUAL(list.contact_size(), 4);

  unordered_map<string, const AddressBookSerialize::Contact*> persons;
...
void TestDeserialization() {
  ...
  AddressBookSerialize::Contact p;
  p.set_name("Vanya");
  ...
  *list.add_contact() = p;
  ...
  istringstream input(output.str(), std::ios::binary);
  const AddressBook book = DeserializeAddressBook(input);
...
void TestFindNameByPrefix() {
  ...
  auto get_names = [](AddressBook::ContactRange range) {
