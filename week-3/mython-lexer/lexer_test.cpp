...
void TestIds() {
  ...
  ASSERT_EQUAL(lexer.NextToken(),
               Token(TokenType::Id{"Return"}));  // keywords are case-sensitive
...
void TestStrings() {
  ...
  ASSERT_EQUAL(
      lexer.NextToken(),
      Token(TokenType::String{"long string with a double quote \" inside"}));
  ASSERT_EQUAL(lexer.NextToken(),
...
x = 1
  ...
  ASSERT_EQUAL(lexer.NextToken(), Token(TokenType::Newline{}));
  // Пустая строка, состоящая только из пробельных символов не меняет текущий
  // отступ, поэтому следующая лексема — это Id, а не Dedent
